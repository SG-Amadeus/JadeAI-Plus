...
function buildProfessionalSectionContent(section: Section, lang: string = 'en'): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it: any) => `<div>
      ...
      ${it.department ? `<p class="mt-0.5 text-sm text-zinc-600"><span class="font-semibold" style="color:${BLUE}">${lang === 'zh' ? '部门：' : 'Department: '}</span>${esc(it.department)}</p>` : ''}
      ...
      ${it.projects?.length ? `<div class="mt-1 space-y-1">${it.projects.map((proj: any) => `<div><p class="text-sm font-bold" style="color:${BLUE}">${esc(proj.name)}</p>${proj.highlights?.length ? `<ul class="mt-0.5 list-disc pl-5">${buildHighlights(proj.highlights, 'text-sm text-zinc-600')}</ul>` : ''}</div>`).join('')}</div>` : ''}
    </div>`).join('')}</div>`;
  }
