...
export function StandardBlueTemplate({ resume }: { resume: Resume }) {
  ...
  return (
    <div
      className="mx-auto w-full max-w-[210mm] bg-white px-[10mm] py-[5mm] text-[9pt] leading-[1.25] text-[#111827] shadow-lg print:shadow-none"
      style={{ fontFamily: "'Microsoft YaHei', 'Noto Sans SC', 'Arial', sans-serif" }}
    >
      <header className="relative">
        <div className="min-h-[16px] pr-[70px] text-center">
          <h1 className="text-[20pt] font-bold leading-none tracking-[0.06em]" style={{ color: BLUE }}>
            {pi.fullName || (zh ? '姓名' : 'Your Name')}
          </h1>
          {pi.jobTitle && (
            <p className="mt-0 text-[10pt] font-medium" style={{ color: BLUE }}>
              {pi.jobTitle}
            </p>
          )}
        </div>

        <div className="mt-0.5 grid grid-cols-[minmax(0,1fr)_56px] gap-x-3">
          <div className="grid grid-cols-2 gap-x-4 gap-y-0 text-[9pt]">
            {personalItems.map((item) => (
              <div key={`${item.label}-${String(item.value)}`} className="flex min-w-0 items-baseline">
                <span className="mr-0 shrink-0 text-[8pt] font-bold" style={{ color: BLUE }}>
                  ▸
                </span>
                <span className="mr-0 shrink-0 font-medium text-[#111827]">{item.label}:</span>
      ...
      <main>
        {resume.sections
          ...
          .map((section) => (
            <section key={section.id} className="mb-1" data-section>
              <SectionTitle title={section.title} />
              <StandardSectionContent section={section} lang={resume.language} />
            </section>
          ))}
      </main>
    </div>
...
function SectionTitle({ title }: { title: string }) {
  return (
    <div className="mb-0.5 mt-1">
      <h2 className="text-[11pt] font-bold" style={{ color: BLUE }}>
        {title}
      </h2>
      <div className="mt-0 h-[1px] w-full" style={{ backgroundColor: BLUE }} />
    </div>
  );
}
...
function EntryHeader({
  ...
  return (
    <div className="flex min-w-0 items-center text-[10pt] font-bold leading-[1.3]" style={{ color: BLUE }}>
      ...
      {right && <span className="shrink-0 whitespace-nowrap text-[9pt]">{right}</span>}
    </div>
  );
}
  ...
  return (
    <ul className="mt-0 space-y-0">
      {items.map((highlight, index) => (
        <li key={index} className="grid grid-cols-[6px_1fr] text-[9pt] leading-[1.25] text-[#20242c]">
          <span className="text-[8pt] font-bold" style={{ color: BLUE }}>
            •
          </span>
...
function StandardSectionContent({ section, lang }: { section: any; lang?: string }) {
  ...
  if (section.type === 'summary') {
    return (
      <RichText
        ...
        className="text-[9pt] leading-[1.25] text-[#20242c]"
      />
    );
  }
      ...
      <div className="space-y-1">
        {(content.items || []).map((item: any) => (
          <article key={item.id} className="break-inside-avoid">
            <div className="flex min-w-0 items-center text-[10pt] font-bold leading-[1.3]" style={{ color: BLUE }}>
              <span className="max-w-[40%] shrink-0 truncate">{item.company || item.position}</span>
              {item.company && item.department && (
                <>
                  <span className={SEP} />
                  <span className="max-w-[34%] shrink-0 truncate">{item.department}</span>
                </>
              )}
              {item.company && item.position && (
                <>
                  <span className={SEP} />
                  <span className="max-w-[34%] shrink-0 truncate">{item.position}</span>
                </>
              )}
              <span className={SEP} />
              <span className="shrink-0 whitespace-nowrap text-[9pt]">
                <DateRange item={item} lang={lang} />
              </span>
            </div>
            <RichText html={item.description} className="mt-0 text-[9pt] leading-[1.25] text-[#20242c]" />
            {item.technologies?.length > 0 && (
              <p className="mt-0 text-[8pt] text-[#374151]">
              ...
              <div className="mt-0.5 space-y-0.5">
                {item.projects.map((proj: WorkProject) => (
                  <div key={proj.id} className="break-inside-avoid">
                    <p className="text-[9pt] font-semibold" style={{ color: BLUE }}>
                      {proj.name}
                    </p>
        ...
        {(content.items || []).map((item: any) => (
          <article key={item.id} className="break-inside-avoid">
            ...
            {(item.gpa || item.description) && (
              <p className="mt-0 text-[8pt] leading-[1.25] text-[#20242c]">
                {item.gpa && (
                  <>
                    <span className="font-semibold" style={{ color: BLUE }}>
                      GPA：
                ...
                {item.gpa && item.description && <span className="mx-1 text-[#9ca3af]">|</span>}
    ...
    return (
      <div className="space-y-0">
        {(content.categories || []).map((category: any) => (
          <p key={category.id} className="text-[9pt] leading-[1.25] text-[#20242c]">
            {category.name && (
              <span className="font-semibold" style={{ color: BLUE }}>
                {category.name}：
              </span>
        ...
        {items.map((item: any) => (
            <article key={item.id} className="break-inside-avoid">
              <div className="flex min-w-0 items-center text-[10pt] font-bold leading-[1.3]" style={{ color: BLUE }}>
                ...
                {item.url && (
                  <>
                    <span className="mx-0.5 h-px w-2 shrink-0 bg-[#1F4E79]" />
                    <span className="max-w-[45%] shrink-0 truncate text-[8pt] font-normal">{item.url}</span>
                  </>
                )}
                <span className={SEP} />
                <span className="shrink-0 whitespace-nowrap text-[9pt]">
  ...
  if (section.type === 'languages') {
    ...
    return (
      <div className="flex flex-wrap gap-x-4 gap-y-0 text-[9pt]">
        {items.map((item: any) => (
          <span key={item.id}>
            <span className="font-semibold" style={{ color: BLUE }}>
              {item.language}：
